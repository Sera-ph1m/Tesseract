  ...
  while (stack.length > 0) {
    ...
    if (indexMap.length == smallerScale.length) {
      ...
      const highIndex: number =
        largerScale.length - smallerScale.length + indexMap.length;
...
function removeRedundantPins(pins: NotePin[]): void {
  for (let i: number = 1; i < pins.length - 1; ) {
    if (
      pins[i - 1].interval == pins[i].interval &&
          ...
          randomNumber4 *
            Math.abs(
              2 *
                Math.floor(
                  Math.sin(
                    (i / randomNumber2) * randomNumber1 + randomNumber3
                  ) *
        ...
        mod(
          24 *
            sigma(
              i,
              (i) => randomNumber1,
              Math.round(randomNumber2 / randomNumber3)
            ),
          24.0000000000001
        )
      )
  ...
  constructor(doc: SongDocument, newValue: number) {
    ...
    if (oldValue != newValue) {
      ...
      const preset: Preset | null =
        preset1 ?? EditorConfig.valueToPreset(newValue);
      if (preset != null) {
  ...
  constructor(doc: SongDocument, usesCurrentInstrumentType: boolean) {
    ...
    function selectCurvedDistribution(
      ...
      for (let i: number = min; i <= max; i++) {
        entries.push({
          item: i,
          weight: 1.0 / (Math.pow((i - peak) / width, 2.0) + 1.0),
        });
      }
    ...
    if (isNoise) {
        ...
        if (instrument.pitchShift != Config.pitchShiftCenter) {
          ...
          instrument.addEnvelope(
            ...
            Config.perEnvelopeSpeedIndices[
              selectCurvedDistribution(1, 63, 50, 13)
            ]
          );
        }
      }
      ...
      if (Math.random() < 0.5) {
        ...
        instrument.reverb = selectCurvedDistribution(
          1,
      ...
      if (type == InstrumentType.noise || type == InstrumentType.spectrum) {
        ...
        if (
          ...
          instrument.addEnvelope(
            ...
            Config.perEnvelopeSpeedIndices[
              selectCurvedDistribution(1, 63, 57, 6)
            ]
          );
      }
        ...
        case InstrumentType.spectrum:
          {
            ...
            const generator =
              spectrumGenerators[
                (Math.random() * spectrumGenerators.length) | 0
              ];
      ...
      if (Math.random() < 0.1) {
        ...
        if (Math.random() < 0.3) {
          ...
          instrument.addEnvelope(
            ...
            Config.perEnvelopeSpeedIndices[
              selectCurvedDistribution(1, 63, 45, 20)
            ],
            envelopeLowerBound,
            envelopeUpperBound,
            ...
            selectWeightedRandom([
              ...
              { item: LFOEnvelopeTypes.triangle, weight: 5 },
            ])
          );
        }
      }
      ...
      if (Math.random() < 0.1) {
        ...
        if (Math.random() < 0.3) {
          instrument.addEnvelope(
            Config.instrumentAutomationTargets.dictionary["bitcrusherFrequency"]
              .index,
            0,
            Config.newEnvelopes.dictionary[
              selectWeightedRandom([
            ...
            Config.perEnvelopeSpeedIndices[
              selectCurvedDistribution(1, 63, 20, 34)
            ],
            envelopeLowerBound,
            envelopeUpperBound,
            selectCurvedDistribution(2, 16, 2, 6),
            ...
            selectWeightedRandom([
              ...
              { item: LFOEnvelopeTypes.triangle, weight: 1 },
            ])
          );
        }
        if (Math.random() < 0.5) {
        ...
        if (instrument.echoSustain != 0 || instrument.echoDelay != 0) {
          ...
          if (Math.random() < 0.04) {
            ...
            instrument.addEnvelope(
              ...
              selectWeightedRandom([
                { item: LFOEnvelopeTypes.sine, weight: 8 },
                { item: LFOEnvelopeTypes.triangle, weight: 3 },
              ])
            );
          }
        }
      ...
      if (Math.random() < 0.07) {
        ...
        if (
          instrument.ringModulation != 0 ||
          instrument.ringModulationHz != 0
        ) {
          ...
          if (Math.random() < 0.1) {
            ...
            instrument.addEnvelope(
              Config.instrumentAutomationTargets.dictionary["ringModulation"]
                .index,
              0,
              Config.newEnvelopes.dictionary[
                selectWeightedRandom([
              ...
              Config.perEnvelopeSpeedIndices[
                selectCurvedDistribution(1, 63, 25, 20)
              ],
              envelopeLowerBound,
              envelopeUpperBound,
              selectCurvedDistribution(2, 16, 2, 6),
          ...
          if (Math.random() < 0.3) {
            ...
            instrument.addEnvelope(
              Config.instrumentAutomationTargets.dictionary["ringModulationHz"]
                .index,
              0,
              Config.newEnvelopes.dictionary[
                selectWeightedRandom([
      ...
      if (Math.random() < 0.2) {
        ...
        instrument.addEnvelope(
          ...
          selectWeightedRandom([
            ...
            { item: RandomEnvelopeTypes.pitch, weight: 2 },
          ])
        );
      }
      function normalize(harmonics: number[]): void {
        ...
        case InstrumentType.fm:
          {
            ...
            for (
              ...
              if (
                ...
                let envelopeLowerBound =
                  selectCurvedDistribution(0, 20, 8, 5) / 10;
                let envelopeUpperBound =
                  selectCurvedDistribution(0, 20, 8, 5) / 10;
                if (envelopeLowerBound >= envelopeUpperBound) {
                  envelopeLowerBound = 0;
                  envelopeUpperBound = 1;
        ...
        case InstrumentType.customChipWave:
          {
            ...
            let randomGeneratedArrayIntegral: Float32Array = new Float32Array(
              65
            );
...
export class ChangeCreateChannelTag extends UndoableChange {
  ...
  protected _doBackwards(): void {
    const index = this._doc.song.channelTags.findIndex(
      (tag) => tag.id === this._tag.id
    );
    if (index > -1) {
...
export class ChangeChannelOrder extends Change {
  constructor(
    ...
    const channelToNewIndexMap = new Map<Channel, number>();
    ...
    const remap = (oldIndex: number): number => {
      ...
      return channelToNewIndexMap.get(channelObject) ?? oldIndex;
    };
  ...
  constructor(
    ...
    if (
      ...
      changeGroup(
        ...
        newNoiseChannelCount + newPitchChannelCount,
        doc.song.pitchChannelCount + doc.song.noiseChannelCount,
        0,
        false,
        true
      ...
      doc.channel = Math.min(
        doc.channel,
        newPitchChannelCount + newNoiseChannelCount + newModChannelCount - 1
      );
      ...
      for (
        ...
        for (
          ...
          instrumentIdx < doc.song.channels[channelIndex].instruments.length;
          instrumentIdx++
        ) {
          for (let mod: number = 0; mod < Config.modCount; mod++) {
            let instrument: Instrument =
              doc.song.channels[channelIndex].instruments[instrumentIdx];
            let modChannel: number = instrument.modChannels[mod];

            if (
              (modChannel >= doc.song.pitchChannelCount &&
                modChannel < oldPitchCount) ||
              modChannel >=
                doc.song.pitchChannelCount + doc.song.noiseChannelCount
            ) {
            ...
            if (
              modChannel >= oldPitchCount &&
              oldPitchCount < newPitchChannelCount
            ) {
              instrument.modChannels[mod] +=
                newPitchChannelCount - oldPitchCount;
            }
          }
        }
      }
...
export class ChangeRemoveChannel extends Change {
  ...
  constructor(doc: SongDocument, index: number) {
    ...
    if (doc.channel >= this._index && doc.song.channels.length > 1) {
      if (
        doc.channel == this._index &&
        doc.channel == doc.song.channels.length - 1
      ) {
        doc.channel--;
      }
    }
...
export class ChangeChannelBar extends Change {
  constructor(
    ...
    if (doc.song.getChannelIsMod(doc.channel)) {
      const pattern: Pattern | null = doc.song!.getPattern(
        doc.channel,
        doc.bar
      );
      if (pattern != null)
...
export class ChangeSongFilterSettings extends UndoableChange {
  ...
  constructor(
    ...
    oldSettings: FilterSettings,
    subFilters: (FilterSettings | null)[] | null = null,
    oldSubFilters: (FilterSettings | null)[] | null = null
  ) {
    super(false);
    this._doc = doc;
  ...
  constructor(doc: SongDocument) {
    ...
    for (
      ...
      for (
        ...
        instrumentIndex < doc.song.channels[channelIndex].instruments.length;
        instrumentIndex++
      ) {
        for (let mod: number = 0; mod < Config.modCount; mod++) {
          let instrument: Instrument =
            doc.song.channels[channelIndex].instruments[instrumentIndex];
          let modInstrument: number = instrument.modInstruments[mod];
          let modChannel: number = instrument.modChannels[mod];

          if (
            modChannel == doc.channel &&
            modInstrument >=
              doc.song.channels[modChannel].instruments.length - 1
          ) {
...
export class ChangeInstrumentsFlags extends Change {
  constructor(
    doc: SongDocument,
    newLayeredInstruments: boolean,
    newPatternInstruments: boolean
  ) {
    super();
    ...
    if (
      oldLayeredInstruments == newLayeredInstruments &&
      oldPatternInstruments == newPatternInstruments
    )
      return;
...
export class ChangeAppendInstrument extends ChangeGroup {
  constructor(doc: SongDocument, channel: Channel, instrument: any) {
    super();
    let newInstrument: Instrument = new Instrument(
      instrument["isDrum"],
      instrument["isMod"]
    );
...
export class ChangeModChannel extends Change {
  constructor(
    ...
    let instrument: Instrument =
      doc.song.channels[doc.channel].instruments[doc.getCurrentInstrument()];
    if (useInstrument != undefined) instrument = useInstrument;
...
export class ChangeModSetting extends Change {
  constructor(doc: SongDocument, mod: number, text: string) {
    ...
    if (tgtChannel >= 0) {
      if (
        instrument.modInstruments[mod] ==
        doc.song.channels[tgtChannel].instruments.length
      ) {
        usedInstruments = usedInstruments.concat(
          doc.song.channels[tgtChannel].instruments
        );
      } else if (
        instrument.modInstruments[mod] >
        doc.song.channels[tgtChannel].instruments.length
      ) {
        let tgtPattern: Pattern | null = doc.song.getPattern(
          tgtChannel,
          doc.bar
        );
        if (tgtPattern != null) {
          for (let i: number = 0; i < tgtPattern.instruments.length; i++) {
            usedInstruments.push(
              doc.song.channels[tgtChannel].instruments[
                tgtPattern.instruments[i]
              ]
            );
          }
        }
      ...
      for (let i: number = 0; i < usedInstruments.length; i++) {
        ...
        if (
          !(
            ...
            (1 << Config.modulators.dictionary[text].associatedEffect)
          )
        ) {
          doc.record(
            new ChangeToggleEffects(
              doc,
              Config.modulators.dictionary[text].associatedEffect,
              tgtInstrument
            )
          );
        }
...
export class ChangeModFilter extends Change {
  constructor(doc: SongDocument, mod: number, type: number) {
    ...
    if (instrument.modFilterTypes[mod] != type) {
      ...
      let cap: number = doc.song.getVolumeCapForSetting(
        true,
        instrument.modulators[mod],
  ...
  constructor(doc: SongDocument, channelIndex: number, bar: number) {
    ...
    if (channelIndex < doc.song.pitchChannelCount + doc.song.noiseChannelCount)
      this._newPatternInstruments =
        doc.recentPatternInstruments[channelIndex].concat();
    else this._newPatternInstruments = [doc.viewedInstrument[channelIndex]];
      ...
      for (;;) {
        if (!setStart) {
          ...
          if (time * direction == bendEnd * direction) {
            ...
            push(
              makeNotePin(persist ? bendTo : oldPin.interval, time, oldPin.size)
            );
            break;
          }
        }
...
export class ChangeReplacePatterns extends ChangeGroup {
  constructor(
    ...
    const combinedChannels: Channel[] = pitchChannels.concat(
      noiseChannels.concat(modChannels)
    );
    for (
      let channelIndex: number = 0;
...
export class ChangeKeyOctave extends Change {
  constructor(doc: SongDocument, oldValue: number, newValue: number) {
    super();
    doc.song.octave = Math.max(
      Config.octaveMin,
      Math.min(Config.octaveMax, Math.round(newValue))
    );
    doc.notifier.changed();
    if (oldValue != newValue) this._didSomething();
  }
...
export class ChangeDragSelectedNotes extends ChangeSequence {
  constructor(
    ...
    if (newStart == newEnd) {
      this.append(
        new ChangeNoteTruncate(doc, pattern, oldStart, oldEnd, null, true)
      );
    } else if (parts < 0) {
      this.append(
        new ChangeNoteTruncate(
  ...
  constructor(doc: SongDocument, pattern: Pattern, scaleMap: number[]) {
    ...
    for (const note of pattern.notes) {
      ...
      for (let i: number = 0; i < note.pitches.length; i++) {
        ...
        const transformedPitch: number =
          scaleMap[pitch % 12] + (pitch - (pitch % 12));
      ...
      for (const oldPin of note.pins) {
        ...
        newPins.push(
          makeNotePin(
            transformedInterval - newPitches[0],
            oldPin.time,
            oldPin.size
          )
        );
  ...
  constructor(doc: SongDocument, index: number) {
    ...
    for (let i: number = index; i < instrument.envelopeCount; i++) {
      ...
      instrument.envelopes[i].pitchEnvelopeEnd =
        instrument.envelopes[i + 1].pitchEnvelopeEnd;
      instrument.envelopes[i].inverse = instrument.envelopes[i + 1].inverse;
      instrument.envelopes[i].perEnvelopeSpeed =
        instrument.envelopes[i + 1].perEnvelopeSpeed;
