...
export class MuteEditor {
	...
	public readonly _channelNameInput: InputBox = new InputBox(
		...
		(oldValue: string, newValue: string) =>
			new ChangeChannelName(this._doc, this._activeChannelIndexForMenu!, oldValue, newValue)
	);
	...
	public openTagContextMenu(tagId: string, event: MouseEvent): void {
		...
		// Position is now relative to the viewport since the menu is in the body
	...
	private _onTagMenuClick = (event: MouseEvent): void => {
		...
		switch (action) {
			...
			case "tagMute": {
				...
				const allMuted = chs.every(
					i => this._doc.song.channels[i].muted
				);
		...
		switch (action) {
			...
			case "chnSolo": {
				...
				for (
					...
					if (
						this._doc.song.channels[chi].type === ChannelType.Mod
					)
						continue;
					if (
						this._doc.song.channels[chi].muted ==
						(chi == ch)
					) {
						shouldSolo = true;
						break;
